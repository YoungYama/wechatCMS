...
public class SysUserServiceImpl implements SysUserService {
	...
	public ResultData<Void> register(SysUser entity) {
		...
		try {
			...
			entity.setUserName(String.valueOf(new Date().getTime()));
			entity.setPassword(EncryptionUtil.encodePassword(entity.getPassword()));
			entity.setUserRole(1);//系统用户
			entity.setRegisterTime(TimeUtil.getCurrentTimeString());
	...
	public ResultData<SysUser> login(SysUser entity, HttpServletRequest request, HttpServletResponse response,
		...
		String password = EncryptionUtil.encodePassword(entity.getPassword());
		SysUser sysUser = sysUserDao.selectByPhoneOrEmail(entity);
			...
			if (password.equals(sysUser.getPassword())) {
				...
				for (Cookie cookie : cookies) {
					if (loginAccount.equals(cookie.getName())) {
						isExist = true;
						String errorCount = String.valueOf(Integer.parseInt(cookie.getValue()) + 1);//记录密码错误次数
						coo = new Cookie(loginAccount, errorCount);
						coo.setPath("/");
				 ...
				 if (!isExist) {
					coo = new Cookie(loginAccount, String.valueOf(1));
					coo.setPath("/");
					coo.setMaxAge(60 * 60);// 一个小时
					response.addCookie(coo);
				}
